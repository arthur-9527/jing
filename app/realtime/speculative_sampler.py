...
class SpeculativeRequest:
    ...
    result: Optional[dict] = None                          # 完整结果（完成后构建）
...
class SpeculativeSampler:
    ...
    async def _run_speculative_llm(self, request: SpeculativeRequest):
        ...
        try:
            ...
            # ⭐ 构建完整结果（供外部通过 request.result 访问）
            request.result = {
                "expression": request.expression,
                "action_events": request.action_events,
                "matched_motion_by_phrase": request.matched_motion_by_phrase,
                "turn_context": request.turn_context,
                "first_pass": request.first_pass,
