...
# 添加 pipecat 路径
PIPECAT_PATH = Path(__file__).parent.parent.parent / "pipecat"
if PIPECAT_PATH.exists():
    sys.path.insert(0, str(PIPECAT_PATH))

from app.realtime.agent_ws_manager import AgentWSManager, AgentStatus
from app.realtime.lipsync_service import LipSyncService, LipMorph
from app.realtime.frame_queue import FrameQueueManager, IdleScheduler, MorphFrame, keyframe_to_vpd
from app.realtime.state_manager import StateManagerProcessor, AgentState
from app.realtime.init_gate import get_init_gate
from app.realtime.panel_manager import PanelManager
...
class AgentService:
    ...
    def __init__(
        ...
        self._running = False
        self._character_id: Optional[str] = None  # ⭐ 角色标识（用于数据隔离）
    ...
    async def initialize(self):
        ...
        # ⭐ 注册口型帧开始推送回调（设置 TTS 基准时间，用于动作时间戳匹配）
        self._frame_queue.set_lip_frames_started_callback(self._on_lip_frames_started)
        logger.info("[AgentService] 口型帧开始推送回调已注册")
        ...
        self._idle_scheduler = IdleScheduler(
            ...
            db_session_factory=get_database().get_session,
            min_interval=10.0,
            max_interval=30.0,
        )
    ...
    async def _init_pipeline(self):
        ...
        try:
            ...
            # 千问 ASR（唯一支持）- 使用新 Provider
            from app.providers.asr import QwenASRProvider, QwenASRSettings
            ...
            logger.info(f"[AgentService] 千问 ASR Provider 已初始化: model={settings.QWEN_ASR_MODEL}")
            
            # LLM 服务 (EmotionalAgent)
            from app.realtime.llm_service import EmotionalAgentLLMService
            ...
            # ⭐ 动态静音策略
            from app.realtime.mute_strategy import get_mute_strategy
            ...
            # ⭐ 先加载角色配置获取 character_id
            from app.agent.character.loader import load_character
            character_config = load_character(settings.CHARACTER_CONFIG_PATH)
            character_id = character_config.character_id
            
            # ⭐ 保存 character_id 到实例属性（用于 handle_interrupt 等方法）
            self._character_id = character_id
            logger.info(f"[AgentService] 角色标识已设置: {character_id}")
            
            # ⭐ 先获取 ConversationBuffer（用于 Redis 历史和 AI 回复写入）
            # 包含 character_id 实现数据隔离
            conversation_buffer = await get_conversation_buffer(
                user_id="default_user",
                character_id=character_id,  # ⭐ 传递 character_id 实现隔离
            )
            ...
            async def _memory_extractor(messages: list[dict]) -> None:
                """批量提取用户信息并写入数据库"""
                from app.agent.memory.writer import extract_and_write_key_events
                ...
                for i in range(0, len(messages) - 1, 2):
                    ...
                    if user_msg and assistant_msg:
                        if user_msg.get("role") == "user" and assistant_msg.get("role") == "assistant":
                            try:
                                await extract_and_write_key_events(
            ...
            # 4. TTS 服务 - CosyVoice WebSocket TTS（唯一支持）
            self._tts_provider = "cosyvoice"
            logger.info(f"[AgentService] 设置 TTS provider: cosyvoice")

            # CosyVoice WebSocket TTS - 使用新 Provider
            from app.providers.tts import CosyVoiceTTSProvider

            logger.info("[AgentService] 使用 CosyVoice TTS Provider")
            tts = CosyVoiceTTSProvider(
                api_key=dashscope_api_key,
                model=settings.COSYVOICE_MODEL,
                clone_voice_audio_path=settings.COSYVOICE_CLONE_AUDIO,
            ...
            from app.realtime.audio_mute_filter import AudioMuteFilter
    ...
    async def start(self):
        ...
        from app.realtime.playback.scheduler import PlaybackScheduler
    ...
    async def handle_interrupt(self):
        ...
        # ⭐ 将打断记录写入聊天记录（使用保存的 character_id 实现隔离）
        from app.services.chat_history import get_conversation_buffer
        try:
            conversation_buffer = await get_conversation_buffer(
                user_id="default_user",
                character_id=self._character_id or "default",  # ⭐ 使用保存的角色标识
            )
        ...
        # ⭐ 取消所有投机采样请求
        try:
            from app.realtime.speculative_sampler import get_speculative_sampler
    ...
    async def _start_idle_scheduling(self):
        ...
        from app.realtime.frame_queue.idle_scheduler import IDLE_DETECT_DELAY
    ...
    async def _on_lip_frames_started(self):
        """口型帧开始推送时设置 TTS 基准时间"""
        self._tts_start_time = time.monotonic()
        logger.info(f"[ActionTiming] TTS 基准时间已设置（口型帧开始推送）")
...
def get_agent_service() -> 'AgentService':
    ...
    if _agent_service is None:
        from app.realtime.agent_ws_manager import agent_ws_manager
