...
class EmotionalAgentLLMService(LLMService):
    ...
    async def _init_agent(self):
        ...
        try:
            ...
            from app.stone import init_database
            ...
            # ⭐ 初始化好感度系统
            await self._init_affection_system(config)
    ...
    async def _init_affection_system(self, config: Any = None):
        """初始化好感度系统并注入到 EmotionalAgent

        ⭐ 初始化内容：
        - AffectionService: 三维好感度状态管理
        - AffectionContextManager: 动态好感度语境生成 + 缓存
        - 加载角色人设文本（personality.md）
        """
        try:
            ...
            # ⭐ Stone 迁移：使用 AffectionRepository 替代 redis_client
            from app.stone import get_affection_repo
            affection_repo = get_affection_repo()

            # 初始化 AffectionService（传入 affection_repo）
            affection_service = AffectionService(affection_repo=affection_repo)
            self._agent._affection_service = affection_service
            logger.info("[EmotionalAgentLLM] AffectionService 已注入到 Agent")
            
            # 初始化 AffectionContextManager
            affection_context_manager = AffectionContextManager(
                affection_service=affection_service,
                llm_client=self._agent.llm,
                character_config=config,
            )
            self._agent._affection_context_manager = affection_context_manager
            logger.info("[EmotionalAgentLLM] AffectionContextManager 已注入到 Agent")

            # 注入到好感度语境刷新调度器
            from app.services.affection.scheduler import set_context_manager
            set_context_manager(affection_context_manager)
            logger.info("[EmotionalAgentLLM] ContextManager 已注入到调度器")
            
            # ⭐ 加载角色人设文本（从 personality.md）
            # 正确解析路径：支持相对路径和绝对路径
            character_dir = self._character_config_path.replace(".json", "")
            # 如果是绝对路径，直接使用；如果是相对路径，基于项目根目录
            import os
            ...
            try:
                ...
                if os.path.exists(personality_path):
                    ...
                    # 解析 personality.md 提取各个部分
                    # 格式：## 性格特点 / ## 情绪特点 / ## 敏感词汇
                    sections = personality_content.split("##")
                    for section in sections:
                        section_lower = section.lower()
                        if "性格" in section_lower or "人设" in section_lower:
                            personality_text = section.strip()
                        elif "情绪" in section_lower or "情感" in section_lower:
                            emotion_traits_text = section.strip()
                        elif "敏感" in section_lower or "触发" in section_lower:
                            emotion_triggers_text = section.strip()

                    self._agent._personality_text = personality_text
                    self._agent._emotion_traits_text = emotion_traits_text
                    self._agent._emotion_triggers_text = emotion_triggers_text

                    logger.info(
                        "[EmotionalAgentLLM] 角色人设已加载: personality=%d chars, emotion_traits=%d chars",
                        len(personality_text),
                        len(emotion_traits_text)
                    )
            except Exception as e:
                logger.warning("[EmotionalAgentLLM] 加载 personality.md 失败: %s，使用默认人设", e)

            # ⭐ 将好感度依赖注入到 EmotionService（内置评估，避免重复回调）
            self._agent.emotion.set_affection_deps(
                affection_service=affection_service,
                llm_chat_fn=self._agent.llm.chat,
                personality_text=personality_text,
                emotion_traits_text=emotion_traits_text,
                emotion_triggers_text=emotion_triggers_text,
            )

            logger.info("[EmotionalAgentLLM] 好感度系统初始化完成")
            
        except Exception as e:
            logger.error(f"[EmotionalAgentLLM] 好感度系统初始化失败: {e}")
            import traceback
            traceback.print_exc()
    ...
    async def _process_context(self, messages: list):
        ...
        # ⭐ 获取投机采样器
        from app.realtime.speculative_sampler import get_speculative_sampler, SpeculativeRequest
        sampler = get_speculative_sampler()
