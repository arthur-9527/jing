...
class FrameQueueManager:
    ...
    def __init__(
        ...
        # ⭐ 口型帧开始推送回调（设置 TTS 基准时间，用于动作触发）
        self._lip_frames_started_callback: Optional[callable] = None
    ...
    def set_lip_frames_started_callback(self, callback: callable) -> None:
        """设置口型帧开始推送回调（设置 TTS 基准时间）

        Args:
            callback: 回调函数，签名 async def callback()
        """
        self._lip_frames_started_callback = callback
        logger.info("[FrameQueue] 口型帧开始推送回调已设置")
                        ...
                        if audio_slice:
                            ...
                            if morphs:
                                ...
                                if not self._has_pushed_lip_frames:
                                    ...
                                    if self._lip_frames_started_callback:
                                        try:
                                            cb = self._lip_frames_started_callback
                                            if asyncio.iscoroutinefunction(cb):
                                                asyncio.create_task(cb())
                                            else:
                                                cb()
                                        except Exception as e:
                                            logger.error(f"[FrameQueue] 口型帧开始推送回调失败: {e}")
