#!/usr/bin/env python3
"""CosyVoice WebSocket API 流式 TTS Provider

继承 BaseTTSProvider，直接用于 Pipecat Pipeline。
...
from app.realtime.lipsync_service import LipSyncService
...
class CosyVoiceTTSProvider(BaseTTSProvider):
    """CosyVoice 流式语音合成 Provider (WebSocket 实现)

    继承 BaseTTSProvider → WebsocketTTSService，直接用于 Pipecat Pipeline。
    实现真正的流式 TTS：
    ...
    # DashScope HTTP TTS API URL
    COSYVOICE_HTTP_URL = "https://dashscope.aliyuncs.com/api/v1/services/audio/tts"
    ...
    async def synthesize(
        ...
        使用 DashScope CosyVoice HTTP API 进行语音合成。
        ...
        # 添加情绪指令（如果有）
        if self._current_instruct_text:
            request_body["parameters"]["instruct_text"] = self._current_instruct_text
            logger.debug(f"[CosyVoice TTS] 非实时合成包含情绪指令: {self._current_instruct_text}")
        ...
        logger.info(f"[CosyVoice TTS] 非实时合成请求: model={self._model}, text_len={len(text)}, voice={self._effective_voice}")
        
        # 发送 HTTP 请求
        async with httpx.AsyncClient(timeout=120.0) as client:
            response = await client.post(
                self.COSYVOICE_HTTP_URL,
                json=request_body,
                headers=headers,
            )
            ...
            if response.status_code != 200:
                ...
                logger.error(f"[CosyVoice TTS] 非实时合成失败: {response.status_code} - {error_msg}")
                raise httpx.HTTPError(f"TTS API 错误: {response.status_code} - {error_msg}")
            
            result = response.json()
            
            # 解析结果 - CosyVoice HTTP API 返回音频数据的 Base64 编码
            output = result.get("output", {})
            audio_base64 = output.get("audio", "")
            
            if not audio_base64:
                logger.error("[CosyVoice TTS] 非实时合成结果无音频数据")
                raise ValueError("TTS API 返回无音频数据")
            
            # Base64 解码
            audio_data = base64.b64decode(audio_base64)
            
            logger.info(f"[CosyVoice TTS] 非实时合成成功: {len(audio_data)} bytes")
            
            return audio_data


# 向后兼容别名
CosyVoiceTTSService = CosyVoiceTTSProvider
