#!/usr/bin/env python3
"""千问实时语音识别 Provider (Qwen ASR)

继承 BaseASRProvider，直接用于 Pipecat Pipeline。
基于阿里云 DashScope 千问实时语音识别 WebSocket API 实现。

提供两类接口：
1. 实时接口（继承自 WebsocketSTTService）：WebSocket 流式识别
2. 非实时接口（transcribe）：HTTP API 文件识别

参考文档：
- 实时 ASR: https://help.aliyun.com/zh/model-studio/developer-reference/qwen-asr-realtime-api
- 非实时 ASR: https://help.aliyun.com/zh/model-studio/developer-reference/paraformer-api
"""
...
class QwenASRProvider(BaseASRProvider):
    ...
    继承 BaseASRProvider → WebsocketSTTService，直接用于 Pipecat Pipeline。
    ...
    async def _handle_transcription_text(self, data: dict):
        ...
        try:
            from app.realtime.speculative_sampler import is_sentence_end, get_speculative_sampler
    ...
    # DashScope HTTP ASR API URL
    DASHSCOPE_ASR_HTTP_URL = "https://dashscope.aliyuncs.com/api/v1/services/audio/asr"
    
    # 非实时 ASR 模型（Paraformer）
    OFFLINE_ASR_MODEL = "paraformer-v2"
    ...
    async def transcribe(
        ...
        使用 DashScope Paraformer 模型进行文件识别。
        适用于语音消息、音频文件等非实时场景。
        
        Args:
            audio_data: 音频数据（PCM 或 WAV 格式）
            sample_rate: 采样率，默认 16000
            format: 音频格式，默认 "pcm"，支持 "pcm", "wav", "mp3", "opus" 等
        ...
        request_body = {
            ...
            "parameters": {
                "language": str(self._settings.language) if is_given(self._settings.language) else "zh",
            },
        }
        ...
        logger.info(f"[QwenASR] 非实时识别请求: model={self.OFFLINE_ASR_MODEL}, format={format}, size={len(audio_data)} bytes")
        
        # 发送 HTTP 请求
        async with httpx.AsyncClient(timeout=60.0) as client:
            response = await client.post(
                self.DASHSCOPE_ASR_HTTP_URL,
                json=request_body,
                headers=headers,
            )
            ...
            if response.status_code != 200:
                ...
                logger.error(f"[QwenASR] 非实时识别失败: {response.status_code} - {error_msg}")
                raise httpx.HTTPError(f"ASR API 错误: {response.status_code} - {error_msg}")
            ...
            logger.info(f"[QwenASR] 非实时识别成功: {text[:100]}...")
